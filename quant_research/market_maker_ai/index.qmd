---
title: "Adaptive Inventory-Aware Market Making on SOFI — AI-Assisted Development"
author: Mihnea POPA
description: " "
date: June 2026
order: -1
...
$$
\text{Score} = \text{NetPnL} - 0.50\,|q_{\text{end}}| - 0.20\,\overline{|q|} - 50\max(0,\, 0.30 - \text{two-sided ratio}) - 10 \times \text{market orders} - 25 \times \text{violations}.
$$

As with the other strategies on this site, the code was developed iteratively with **Claude (Anthropic)**: I supplied the assignment brief, the previous week's code, and the instructor's feedback, and Claude proposed and implemented the quoting logic, which I then tested locally before submission. All figures below come from the autograder's `summary.json` outputs and the instructor's feedback reports.

### Week 7 baseline: a textbook Avellaneda-Stoikov quoter, and where it breaks
...
1. **Move-gate throttle** — the Level-2 refresh now fires only when both 150 events have passed *and* the NBBO has moved by at least one tick, eliminating churn on flat markets while staying responsive to real price moves.
